...
for(const token of ['class MainActivity : Activity()','WebViewAssetLoader','AssetsPathHandler','javaScriptEnabled = true','domStorageEnabled = true','allowFileAccess = false','allowContentAccess = false','allowFileAccessFromFileURLs = false','allowUniversalAccessFromFileURLs = false','https://appassets.androidplatform.net/assets/www/index.html','installSplashScreen()','override fun onBackPressed()']) assert(activity.includes(token),`MainActivity missing ${token}`);
assert(!activity.includes('onBackPressedDispatcher'),'MainActivity should not require Activity KTX');
